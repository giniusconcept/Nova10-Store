...
function renderDashboard(){
  ...
  setText('mVisitors',num(t.sessions));
  setText('mViews',num(t.pageViews));
  setText('mOrders',num(t.orders));
  setText('mRevenue',euro(t.revenue));
  setText('sessions30',`${num(l.sessions)} sessions`);
  setText('readyProducts',`${d.operational.productsReady} / ${d.operational.productsTotal}`);
  setText('fProductViews',num(l.productViews));
  setText('fCart',num(l.addToCart));
  setText('fCheckout',num(l.checkouts));
  setText('fOrders',num(l.orders));
  setText('fConversion',l.sessions?`${((l.orders/l.sessions)*100).toFixed(2).replace('.',',')} %`:'0 %');
  setText('aVisitors',num(l.sessions));
  setText('aViews',num(l.pageViews));
  setText('aProducts',num(l.productViews));
  setText('aSearches',num(l.searches));
  setText('financeRevenue',euro(t.revenue));
  setText('financeOrders',num(t.orders));
  setText('ordersBadge',num((d.orders||[]).length));
...
function renderOrders(){
  const body=document.getElementById('ordersBody'), empty=document.getElementById('ordersEmpty'); if(!body||!empty)return;
...
function renderCustomers(){
  const host=document.getElementById('customersList'); if(!host)return; const list=DATA.customers||[];
...
async function loadData(showSpinner=true){
  const refresh=document.getElementById('refresh'); if(showSpinner&&refresh){refresh.disabled=true;refresh.textContent='Actualisation…'}
...
function switchView(name){
  ...
  const btn=document.querySelector(`.nav button[data-view="${name}"]`); const title=document.getElementById('topTitle'); if(title)title.textContent=btn?btn.textContent.trim():'NOVA10';
...
async function boot(){
  ...
  document.getElementById('logout')?.addEventListener('click',async()=>{try{await api('/api/admin/logout',{method:'POST',body:'{}'})}catch{}showLogin();const p=document.getElementById('password');if(p)p.value=''});
}
